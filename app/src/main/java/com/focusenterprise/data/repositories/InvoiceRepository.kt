...
class InvoiceRepository(
    ...
    val activeInvoices: Flow<List<Invoice>> = invoiceDao.getActiveInvoices()
    val completedInvoices: Flow<List<Invoice>> = invoiceDao.getCompletedInvoices()
    ...
    fun getCustomerMonthlySummaries(): Flow<List<CustomerMonthlySummary>> {
        return invoiceDao.getCustomerMonthlySummaries()
    }
    ...
    suspend fun addPaymentToInvoice(invoice: Invoice, amount: Double) {
        withContext(Dispatchers.IO) {
            ...
            // Stock deduction happens during invoice creation, not payment
    ...
    suspend fun deleteInvoice(invoice: Invoice) {
        withContext(Dispatchers.IO) {
            ...
            invoiceItemDao.deleteItemsForInvoice(invoice.invoiceId)
            // Delete associated payments
            paymentDao.deletePaymentsForInvoice(invoice.invoiceId)
            // Delete the invoice
