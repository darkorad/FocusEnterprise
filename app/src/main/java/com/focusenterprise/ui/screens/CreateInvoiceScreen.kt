    ...
    Scaffold(
        ...
        LazyColumn(modifier = Modifier.padding(padding).padding(16.dp)) {
            ...
            items(lineItems) { item ->
                val stockItem = viewModel.allStockItems.collectAsState(initial = emptyList()).value.find { it.stockId == item.stockId }
        ...
        if (showAddItemDialog) {
            AddItemDialog(
                stockItems = viewModel.allStockItems.collectAsState(initial = emptyList()).value,
